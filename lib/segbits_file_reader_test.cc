...
TEST(SegbitsFileReaderTest, ZeroLengthFileYieldsNoItems) {
  auto segbits_reader = prjxray::SegbitsFileReader::InitWithFile("empty_file");
...
TEST(SegbitsFileReaderTest, FileWithOneEntryWithEmptyTag) {
  auto segbits_reader =
      prjxray::SegbitsFileReader::InitWithFile("one_entry_empty_tag.segbits");
  ASSERT_TRUE(segbits_reader);
...
TEST(SegbitsFileReaderTest, FileWithOneEntryMissingBit) {
  auto segbits_reader =
      prjxray::SegbitsFileReader::InitWithFile("one_entry_missing_bit.segbits");
  ASSERT_TRUE(segbits_reader);
