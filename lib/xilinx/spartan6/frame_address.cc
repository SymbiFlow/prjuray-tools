...
FrameAddress::FrameAddress(spartan6::BlockType block_type, uint8_t row,
                           uint8_t column, uint16_t minor) {
  address_ = bit_field_set(0, 31, 28, block_type);
  address_ = bit_field_set(address_, 27, 24, row);    // high register, bit 8-11
  address_ = bit_field_set(address_, 23, 16, column); // high register, bits 0-7
  address_ = bit_field_set(address_, 9, 0, minor);    // low register, bit 0-9
}
...
uint8_t FrameAddress::row() const { return bit_field_get(address_, 27, 24); }

uint8_t FrameAddress::column() const { return bit_field_get(address_, 23, 16); }

uint16_t FrameAddress::minor() const { return bit_field_get(address_, 9, 0); }
...
bool convert<spartan6::FrameAddress>::decode(const Node &node,
  ...
  lhs = spartan6::FrameAddress(node["block_type"].as<spartan6::BlockType>(),
