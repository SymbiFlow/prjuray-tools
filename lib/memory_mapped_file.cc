...
MemoryMappedFile::InitWithFile(const std::string &path) {
  ...
  if (statbuf.st_size == 0) {
    close(fd);
    return std::unique_ptr<MemoryMappedFile>(new MemoryMappedFile(nullptr, 0));
  }
...
MemoryMappedFile::~MemoryMappedFile() { munmap(data_, size_); }
