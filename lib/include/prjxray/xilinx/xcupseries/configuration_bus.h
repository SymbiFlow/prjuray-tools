...
public:
  ...
  // address components.
  template <typename T> ConfigurationBus(T first, T last);
...
template <typename T> ConfigurationBus::ConfigurationBus(T first, T last) {
...
namespace YAML {
template <> struct convert<prjxray::xilinx::xcupseries::ConfigurationBus> {
  static Node encode(const prjxray::xilinx::xcupseries::ConfigurationBus &rhs);
