...
public:
  ...
  template <typename T> Row(T first, T last);
...
template <typename T> Row::Row(T first, T last) {
...
namespace YAML {
template <> struct convert<prjxray::xilinx::xcupseries::Row> {
  static Node encode(const prjxray::xilinx::xcupseries::Row &rhs);
  static bool decode(const Node &node, prjxray::xilinx::xcupseries::Row &lhs);
};
} // namespace YAML
