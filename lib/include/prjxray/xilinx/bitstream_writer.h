...
uint32_t
packet2header(const ConfigurationPacket<Spartan6ConfigurationRegister> &packet);
...
public:
  ...
  typedef std::vector<
      std::unique_ptr<ConfigurationPacket<typename ArchType::ConfRegType>>>
      packets_t;
  ...
  protected:
    explicit packet_iterator(
        ...
        state_t state, data_iterator_t itr_data);
...
template <typename ArchType>
bool BitstreamWriter<ArchType>::packet_iterator::
operator==(const packet_iterator &other) const {
...
BitstreamWriter<ArchType>::iterator::iterator(
    ...
    : itr_header_(itr_header), packets_(packets), itr_packets_(itr_packets),
...
template <typename ArchType>
bool BitstreamWriter<ArchType>::iterator::
