...
public:
  ...
  template <typename T> GlobalClockRegion(T first, T last);
...
template <typename T> GlobalClockRegion::GlobalClockRegion(T first, T last) {
...
namespace YAML {
template <> struct convert<prjxray::xilinx::xc7series::GlobalClockRegion> {
  static Node encode(const prjxray::xilinx::xc7series::GlobalClockRegion &rhs);
