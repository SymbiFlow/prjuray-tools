...
template <typename ArchType> class Configuration {
public:
  using FrameMap =
      std::map<typename ArchType::FrameAddress, typename ArchType::FrameData>;
  ...
  static absl::optional<Configuration<ArchType>>
  InitWithPackets(const typename ArchType::Part &part, Collection &packets);
...
Configuration<ArchType>::createType2ConfigurationPacketData(
  ...
  static const int kZeroFramesSeparatorWords = ArchType::words_per_frame * 2;
  for (auto &frame : frames) {
...
Configuration<Spartan6>::InitWithPackets(const typename Spartan6::Part &part,
  ...
  for (auto packet : packets) {
    if (packet.opcode() !=
        ConfigurationPacket<typename ArchType::ConfRegType>::Opcode::Write) {
      continue;
    }
    ...
    case ArchType::ConfRegType::IDCODE: {
      ...
      uint32_t idcode = (packet.data()[0] << 16) | (packet.data()[1]);
    ...
    case ArchType::ConfRegType::FAR_MAJ: {
      ...
      if (packet_size < 1) {
        ...
        frame_address_register = (packet.data()[0] & 0xFFFF) << 16;
      } else {
        frame_address_register =
            ((packet.data()[0] & 0xFFFF) << 16) | (packet.data()[1] & 0xFFFF);
      }
      break;
    }
    case ArchType::ConfRegType::FAR_MIN:
      ...
      frame_address_register |= packet.data()[0] & 0x3FF;
