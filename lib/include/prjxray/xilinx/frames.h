...
template <typename ArchType> class Frames {
public:
...
bool verifyECC(const typename ArchType::FrameData &data);
...
int Frames<ArchType>::readFrames(const std::string &frm_file_str,
  ...
  if (!frm_file) {
    std::cerr << "Unable to open frm file: " << frm_file_str << std::endl;
    return 1;
  }
  std::string frm_line;
  ...
  while (std::getline(frm_file, frm_line)) {
    ...
    uint32_t address = std::stoul(frame_delta.first, nullptr, 16);
    typename ArchType::FrameAddress frame_address(address);
    if (!part.IsValidFrameAddress(frame_address)) {
      std::cerr << "Frames file contains an invalid frame: " << frame_address
                << std::endl;
      return 1;
    }
    ...
    if (!std::is_same<ArchType, Spartan6>::value) {
      if (frame_data_strings.size() != ArchType::words_per_frame) {
        std::cerr << "Frame " << std::hex << address << ": found " << std::dec
                  << frame_data_strings.size() << " words instead of "
    ...
    std::transform(frame_data_strings.begin(), frame_data_strings.end(),
                   frame_data.begin(), [](const std::string &val) -> uint32_t {
...
void Frames<ArchType>::addMissingFrames(
  ...
  auto current_frame_address = absl::optional<typename ArchType::FrameAddress>(
