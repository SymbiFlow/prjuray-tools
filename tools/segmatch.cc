...
void read_input(std::istream &f, std::string filename) {
  ...
  while (f >> token) {
    ...
    if (token == "tag") {
      ...
      auto &tags =
          token == "1" ? segdata_tags1(*segptr) : segdata_tags0(*segptr);

      tags.set(tag_idx);

      if (FLAGS_i) {
        auto &inv_tags =
            token == "1" ? segdata_tags0(*segptr) : segdata_tags1(*segptr);
...
int main(int argc, char **argv) {
  ...
  for (int tag_idx = 0; tag_idx < num_tags; tag_idx++) {
    ...
    if (FLAGS_c < 0 || (0 < num_candidates && num_candidates <= FLAGS_c)) {
