...
#include <prjxray/xilinx/ecc.h>

DEFINE_bool(c, false, "output '*' for repeating patterns");
DEFINE_bool(C, false, "do not ignore the ECC bits in each frame");
DEFINE_int32(f, -1,
...
DEFINE_bool(E, false, "Ignore failing frame ECC verification");
...
struct BitReader {
  ...
  template <typename T> int operator()(T &arg) {
    ...
    auto config =
        xilinx::Configuration<ArchType>::InitWithPackets(*part, *reader);
    if (!config) {
      std::cerr << "Bitstream does not appear to be for this part" << std::endl;
      return 1;
    }

    std::cout << "Number of configuration frames: " << config->frames().size()
              << std::endl;
    ...
    if (!FLAGS_o.empty()) {
      ...
      if (f == nullptr) {
        printf("Can't open output file '%s' for "
               "writing!\n",
               FLAGS_o.c_str());
        return 1;
      }
    ...
    if (!FLAGS_aux.empty()) {
      ...
      if (aux_file == nullptr) {
        printf("Can't open aux output file '%s' for "
               "writing!\n",
               FLAGS_aux.c_str());
        return 1;
      }
      // Extract and decode header information as in RBT file
      xilinx::BitstreamReader<ArchType>::ExtractHeader(bytes_, aux_file);
    ...
    for (auto &it : config->frames()) {
      ...
      if (!xilinx::verifyECC<ArchType>(it.second)) {
        std::stringstream msg;
        msg << "ECC verification of frame " << it.first << " failed.\n";
        if (FLAGS_E) {
      ...
      if (FLAGS_o.empty())
        printf("Frame 0x%08x (Type=%d Top=%d Row=%d "
               "Column=%d "
               "Minor=%d):\n",
               static_cast<uint32_t>(it.first),
               static_cast<unsigned int>(it.first.block_type()),
               it.first.is_bottom_half_rows() ? 1 : 0, it.first.row(),
               it.first.column(), it.first.minor());
      ...
      if (FLAGS_p) {
        ...
        for (int i = 0; i < (int)it.second.size(); i++) {
          for (int k = 0; k < word_length; k++) {
            // Skip ECC bits
            if (!FLAGS_C and xilinx::is_ecc_bit<ArchType>(i, k)) {
              continue;
            }
        ...
        for (size_t i = 0; i < it.second.size(); i++)
          if (std::is_same<ArchType, prjxray::xilinx::Spartan6>::value) {
            fprintf(f, "%08x%s", it.second.at(i) & 0xffffffff,
                    (i % 6) == 5 ? "\n" : " ");
          } else {
            fprintf(
                f, "%08x%s",
                it.second.at(i) &
                    ((i != 50 || FLAGS_C ||
                      !std::is_same<ArchType, prjxray::xilinx::Series7>::value)
                         ? 0xffffffff
                         : 0xffffe000),
                (i % 6) == 5 ? "\n" : " ");
          }
    ...
    if (FLAGS_p) {
      ...
      for (int y = 0, bit = ArchType::words_per_frame * word_length - 1;
           y < height; y++, bit--) {
...
int main(int argc, char **argv) {
  ...
  if (argc == 2) {
    ...
    if (!in_file) {
      std::cerr << "Can't open input file '" << in_file_name << "' for reading!"
                << std::endl;
      return 1;
    }

    std::cout << "Bitstream size: " << in_file->size() << " bytes" << std::endl;

    in_bytes = std::vector<uint8_t>(static_cast<uint8_t *>(in_file->data()),
    ...
    std::cout << "Bitstream size: " << in_bytes.size() << " bytes" << std::endl;
  }
