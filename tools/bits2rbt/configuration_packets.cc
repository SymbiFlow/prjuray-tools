...
ConfigurationPackets::ConfigurationPackets(const std::string &arch)
    : words_per_frame_(words_in_architecture.at(arch)), architecture_(arch),
      ecc_(arch, words_per_frame_) {}

const std::unordered_map<std::string, size_t>
    ConfigurationPackets::words_in_architecture = {
        {"Series7", 101}, {"UltraScale", 123}, {"UltraScalePlus", 93}};

std::shared_ptr<ConfigurationPackets>
ConfigurationPackets::InitFromFile(const std::string &file,
  ...
  if (words_in_architecture.find(arch) == words_in_architecture.end()) {
    throw std::runtime_error("Error: Unrecognized architecture " + arch + "\n");
  }
...
void ConfigurationPackets::WriteFpgaConfiguration(
    std::ostream &out, const std::vector<uint32_t> &vect) {
  for (auto &word : vect) {
...
void ConfigurationPackets::WritePacket(
    std::ostream &out, ConfigurationFrames::const_iterator frame_citr) const {
  bool is_new_row(false);
  if ((frame_citr == configuration_data_packets_.cbegin()) ||
      IsDifferentRow(frame_citr, std::prev(frame_citr))) {
    // Write the WCFG command followed by a FAR write with address of the next
    // frame WCFG command write
...
void ConfigurationPackets::Line2Vector(const std::string &line,
  ...
  std::transform(str_vector.begin(), str_vector.end(), std::back_inserter(vect),
                 str_to_uint);
}
...
uint32_t ConfigurationPackets::GetFpgaConfigurationCRC() const {
  ...
  auto far = std::search(fpga_configuration_head_.begin(),
                         ...
                         kRCrcCmd.end());
  for (auto itr = far + kRCrcCmd.size(); itr != fpga_configuration_head_.end();
       ++itr) {
    if (*itr == kNop)
      continue;
...
uint32_t
ConfigurationPackets::GetCRC(ConfigurationFrames::const_iterator frame_citr,
...
void ConfigurationPackets::InitializeHeader(const std::string &line) {
  // FIXME Remove the configuration data head part once the aux data is fixed
...
bool ConfigurationPackets::IsDifferentRow(
  ...
  auto get_row = [this](const uint32_t &address) {
    size_t row_shift = (architecture_ == "UltraScalePlus") ? 18 : 17;
