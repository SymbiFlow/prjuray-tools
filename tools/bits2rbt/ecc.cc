...
const std::unordered_map<std::string, size_t> ECC::ecc_word_per_architecture = {
    {"Series7", 50}, {"UltraScale", 60}, {"UltraScalePlus", 45}};

uint32_t ECC::GetSeries7WordEcc(uint32_t idx, uint32_t data,
                                uint32_t ecc) const {
...
uint64_t ECC::GetUSEccFrameOffset(int word, int bit) const {
  ...
  // e.g. word 3, bit 9
  // offset: 0b10100110010 - concatenate (3 + (255 - last_frame_index(e.g. 92
  // for US+)) [frame offset] and 9/4 [nibble offset] becomes: 0x10100110010
