...
struct Frames2BitWriter {
  template <typename T> int operator()(T &arg) {
    ...
    if (!part) {
      std::cerr << "Part file " << FLAGS_part_file << " not found or invalid"
                << std::endl;
      return 1;
    }
    ...
    if (frames.readFrames(FLAGS_frm_file, *part)) {
      std::cerr << "Frames file " << FLAGS_frm_file << " not found or invalid"
                << std::endl;
      return 1;
    }
