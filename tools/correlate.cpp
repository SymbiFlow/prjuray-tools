...
int main(int argc, char *argv[]) {
	...
	for(struct dirent *dirent = readdir(dir); dirent != nullptr; dirent = readdir(dir)) {
		...
		if (p.substr(idx) != ".features")
			continue;
		auto stem = p.substr(0, idx);
		std::ifstream tilebits(parent_path + "/" + stem + ".tbits");
		if (!tilebits) {
			std::cerr << "Failed to open " << (parent_path + "/" + stem + ".tbits") << std::endl;
			return 1;
		}
		parse_bits(stem, tilebits);
		std::ifstream features(p);
		if (!features) {
			std::cerr << "Failed to open " << p << std::endl;
			return 1;
		}
		parse_features(stem, features);
	}
