...
int ListConfigPackets(int argc, char *argv[]) {
  ...
  if (argc < 1) {
    ...
    std::cerr << "Usage: " << argv[0] << "list_config_packets <bit_file>"
              << std::endl;
    return 1;
  }
  if (argc == 2) {
...
struct DebugFrameAddressesDumper {
  DebugFrameAddressesDumper(const absl::Span<uint8_t> &bytes) : bytes_(bytes) {}
  ...
  template <typename T> int operator()(T &arg) {
    ...
    for (auto packet : *reader) {
      ...
      if (packet.data().size() != 1) {
        std::cerr << "Write to FAR with word_count != 1" << std::endl;
        continue;
      }
    ...
    if (!found_one_lout) {
      std::cerr << "No LOUT writes found.  Was "
                << "BITSTREAM.GENERAL.DEBUGBITSTREAM set to YES?" << std::endl;
      return 1;
    }
...
int DumpDebugbitstreamFrameAddresses(int argc, char *argv[]) {
  ...
  if (argc < 1) {
    ...
    std::cerr << "Usage: " << argv[0]
              << "dump_debugbitstream_frame_addresses <bit_file>" << std::endl;
    return 1;
  }
  if (argc == 2) {
    architecture = argv[1];
...
struct DeviceIdGetter {
  ...
  template <typename T> int operator()(T &arg) {
    ...
    auto idcode_packet = std::find_if(
        ...
        [](const xilinx::ConfigurationPacket<typename ArchType::ConfRegType>
               &packet) {
          return (packet.opcode() ==
                  xilinx::ConfigurationPacket<
                      typename ArchType::ConfRegType>::Opcode::Write) &&
                 (packet.address() == ArchType::ConfRegType::IDCODE);
        });
...
int GetDeviceId(int argc, char *argv[]) {
  ...
  if (argc < 1) {
    std::cerr << "ERROR: no input specified" << std::endl;
    std::cerr << "Usage: " << argv[0]
              << "get_device_id <bit_file> [<architecture>]" << std::endl;
    return 1;
  }
  if (argc == 2) {
    architecture = argv[1];
...
int main(int argc, char *argv[]) {
  Action actions[] = {
      ...
      {"dump_debugbitstream_frame_addresses", DumpDebugbitstreamFrameAddresses},
  ...
  if (argc < 2) {
    ...
    std::cerr << "Usage: " << argv[0] << "<command> <command_options>"
              << std::endl;
    return 1;
  }
  ...
  if (requested_action == std::end(actions)) {
    std::cerr << "Unknown action: " << requested_action_str << std::endl;
    return 1;
  }
