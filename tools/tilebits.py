...
def get_slice_xy(site):
    """ Get slice XY from SLICE_X<x>Y<y>

    >>> get_slice_xy('SLICE_X0Y0')
    (0, 0)
    >>> get_slice_xy('SLICE_X1Y2')
    (1, 2)
    >>> get_slice_xy('SLICE_X50Y2')
    (50, 2)
    >>> get_slice_xy('SLICE_X50Y199')
    (50, 199)
    """
...
def main():
    ...
    with open(sys.argv[2], 'r') as llf:
        for line in llf:
            ...
            site_x, site_y = get_slice_xy(site)
